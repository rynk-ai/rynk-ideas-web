...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userId) {
      ...
      const user = await db
        .prepare("SELECT subscriptionTier FROM users WHERE id = ?")
        .bind(userId)
        .first();
      ...
      if (tier === "free") {
        ...
        if (dumpCount >= 50) {
          return NextResponse.json(
            {
              error:
                "Monthly limit reached. Please upgrade to Pro for unlimited dumps.",
            },
            { status: 403 },
          );
        }
    ...
    const { content, contentType = "text", mediaUrls }: any = await req.json();
