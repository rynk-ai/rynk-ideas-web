      ...
      <main className="flex-1 overflow-y-auto no-scrollbar scroll-smooth">
        <div className="max-w-3xl mx-auto px-4 md:px-6 py-8 md:py-12 flex flex-col min-h-full pb-32">
          ...
          {(thread.groundingNote || thread.stateReason) && (
            <div
              className="mb-12 grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-slide-up"
              style={{ animationDelay: "50ms" }}
            >
              {thread.groundingNote && (
                <div className="card-swiss p-4 rounded-xl border border-primary/20 bg-primary/5">
                  <div className="swiss-label mb-2 text-primary/70">
                    {t("groundingNote")}
                  </div>
          ...
          <div className="flex-1 flex flex-col">
            <div className="flex items-center justify-between mb-8">
              <h2 className="swiss-label">{t("timeline")}</h2>
              <button
            ...
            {segments.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center py-12 text-center">
                <p className="text-muted-foreground swiss-subhead">
                  {t("noEntries")}
                </p>
                <p className="text-xs text-muted-foreground/60 mt-2">
                  Add a note below to start capturing thoughts.
                </p>
              </div>
            ) : (
              <div className="space-y-8 pb-4">
                {(order === "desc" ? [...segments].reverse() : segments).map(
                  (segment, i) => (
                    <div
                      key={segment.id}
